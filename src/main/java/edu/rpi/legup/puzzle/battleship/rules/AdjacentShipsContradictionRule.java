...
public class AdjacentShipsContradictionRule extends ContradictionRule {
    ...
    public String checkContradictionAt(Board board, PuzzleElement puzzleElement) {
        ...
        //checks to see if ship is a vertical type(meaning ship continuation will be up and down)
        boolean isVertical = (up != null && BattleshipType.isShip(up.getData()))
                || (down != null && BattleshipType.isShip(down.getData()));

        //checks to see if ship is a horizontal type(meaning ship continuation will be left and right)
        ...
        //having passed all adjacent ship checks, we return no error message
        return super.getNoContradictionMessage() + ": " + this.NO_CONTRADICTION_MESSAGE;
    }
}